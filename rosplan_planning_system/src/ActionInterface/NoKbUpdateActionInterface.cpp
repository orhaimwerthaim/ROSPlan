...
namespace KCL_rosplan {
	...
	void NoKbUpdateActionInterface::runActionInterface() {
		...
		if(client.call(srv)) {
			...
			ROS_ERROR("KCL: (NoKbUpdateActionInterface) could not call Knowledge Base for operator details, %s", params.name.c_str());
			return;
		}
		...
		for(; nit!=predicateNames.end(); nit++) {
			...
			if(predClient.call(predSrv)) {
				...
				ROS_ERROR("KCL: (NoKbUpdateActionInterface) could not call Knowledge Base for predicate details, %s", params.name.c_str());
				return;
			}
		}
        ...
        ops.template init<rosplan_dispatch_msgs::ActionDispatch>(adt, 1000, boost::bind(&KCL_rosplan::NoKbUpdateActionInterface::dispatchCallback, this, _1));
        ops.transport_hints = ros::TransportHints();
        ops.allow_concurrent_callbacks = true;
		ros::Subscriber ds = nh.subscribe(ops); // nh.subscribe(adt, 1000, &KCL_rosplan::NoKbUpdateActionInterface::dispatchCallback, this);
	...
	/* run action interface */
	void NoKbUpdateActionInterface::dispatchCallback(const rosplan_dispatch_msgs::ActionDispatch::ConstPtr& msg) {
